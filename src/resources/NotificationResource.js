export default class NotificationResource {
  ...
  registerToken(token) {
    firebase.database().ref('fcmTokens/').push({ token: token, user_id: this.user.uid });
  }
  ...
  saveTokenToServer() {
    this.messaging.getToken().then(res => {
      if (this.tokensLoaded) {
        ...
        if (existingToken) {
          firebase.database().ref(`/fcmTokens/${existingToken}`).set({ token: res, user_id: this.user.uid });
        } else {
